...
Nsamp = 100;     % No. of samples to obtain from the Beta Function
Nsamples = 1000; % No. of samples to obtain from the posterior
upper_bounds = [0.005, 0.500, 5.000]; % Bounds on the Beta Function
...
bounds1 = [0.01, 200]; bounds2 = [0.01, 200]; 
...
rnd = @(x, N) upper_bounds(1).*betarnd(x(:,1), x(:,2), N, 1);
...
rnd = @(x, N) upper_bounds(2).*betarnd(x(:,1), x(:,2), N, 1);
...
rnd = @(x, N) upper_bounds(3).*betarnd(x(:,1), x(:,2), N, 1);
...
save('Cluster_NAFEMS_Problem_2a', 'TEMCMC1_logevi', 'TEMCMC2_logevi', 'TEMCMC3_logevi', 'timeCluster', 'mean_beta', 'std_beta')