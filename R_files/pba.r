...
######################################################################################################
# this range.pbox function creates the error "Error in unique.default(v) : unique() applies only to vectors" so we replace it by the older version below
...
#if (typeof(...)=="list") return(mixture.list(..., w=w))
  el <- list(...)
  k = length(el) 
  A = rep(list(pbox(0)),k)
  for (i in 1:k) A[[i]] = pbox(el[[i]])
...
# My personal crusade against the limitations of object-oriented coding in R is almost complete in that all but one of these calls work even without explicitly calling mixture.pbox:
#
#	par(mfcol=c(3,3))
#	a = N(15,1); b = U(3,9); d = beta(10,1); c = mixture(a,b,d); c
#	a = N(15,1); b = U(3,9); d = beta(10,1); c = mixture(a,b,d,w=c(1,2,3)); c
#	a = N(15,1); b = U(3,9); d = beta(10,1); c = mixture(c(a,b,d),w=c(1,2,3)); c       ############# this doesn�t work
#	a = N(15,1); b = interval(3,9); d = beta(10,1); c = mixture(a,b,d,w=c(1,2,3)); c
#	a = N(15,1); b = interval(3,9); d = beta(10,1); c = mixture(b,a,d,w=c(1,2,3)); c
#	a = N(15,1); b = interval(3,3); d = beta(10,1); c = mixture(b,a,d,w=c(1,2,3)); c
#	a = N(15,1); b = 3;                    d = beta(10,1); c = mixture(b,a,d,w=c(1,2,3)); c
#	a = 15;          b = 6; d = 1;                                    c = mixture(a,b,d); c
#	a = 15;          b = 6; d = 1;                                    c = mixture(a,b,d,w=c(1,2,3)); c
#
# How can I support the one that doesn't work, calling with a list of p-boxes, rather than a splay of them?  Of course, I can explicitly call mixture.list(c(a,b,d)), but that�s annoying.  You would think that I should be able to modify the mixture function to just check, like this: 
#
#	mixture <- function(..., w=NULL) {
#	# if one arg, it is probably a list or array, and w should be a vector of 1's of the same length
#	# if just two args they may be either two mixturands to be evenly mixed, or one array of mixturands and one array of weights
#	if (typeof(...)=="list") return(mixture.list(..., w=w))    #   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< should make it work, right? 
#	el <- list(...)
#	k = length(el) 
#	A = rep(list(pbox(0)),k)
#	for (i in 1:k) A[[i]] = pbox(el[[i]])
...
#	}
#
# But that�s not allowed for some arcane reason.  Apparently, you can�t generally ask for the type of an ellipsis in R.
...
# this constructor for stable distributions assumes scalar parameters; should handle interval parmeters
# it requires the R Package 'stabledist' by Diethelm Wuertz, Martin Maechler and Rmetrics core team members. Computes stable density, probability, quantiles, and random numbers
#https://cran.r-project.org/web/packages/stabledist/stabledist.pdf
# see also https://en.wikipedia.org/wiki/Stable_distribution
#see also http://www.robustanalysis.com/public/stable.html

stable = function(location=0, scale=1, index=1, sk=0, name=''){ 
  #	index must be (0,2]
  # sk must be in [-1,1]
  library('stabledist')
  if ((index <= 0) || (2 < index)) stop('skewness out of range for stable distribution')
  if ((sk < -1) || (1 < sk)) stop('skewness out of range for stable distribution')
  pbox(u=qstable(iii(),index,sk,scale,location), d=qstable(jjj(),index,sk,scale,location), shape='stable',name=name) # ml=, mh=, vl=, vh=
  }

## testing stable
#par(mfrow=c(4,3))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); for (p in 0:10) { s = stable(p); lines(s) }; title('location')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); for (p in 0:10) { s = stable(scale=p); lines(s) }; title('scale')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); for (p in (1:10)/10) { s = stable(index=p); lines(s) }; title('index')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); for (p in 0:10) { s = stable(sk=(p-5)/5); lines(s) }; title('skewness parameter')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); for (p in 0:10) { s = stable(10,2,runif(1,0,2),runif(1,-1,1)); lines(s) }; title('various')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1)); green(cauchy(0,1)); title('cauchy')
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,0)); blue(N(0,1)); title(paste(0))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,0.1)); blue(N(0,1)); title(paste(0.1))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,1)); blue(N(0,1)); title(paste(1))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,1.5)); blue(N(0,1)); title(paste(1.5))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,1.99)); blue(N(0,1)); title(paste(1.99))
#plot(NULL,xlim=c(-10,30),ylim=c(0,1)); red(stable(0,1,2)); blue(N(0,1)); title(paste(2))
